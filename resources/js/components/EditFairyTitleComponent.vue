    ...
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div class="col-md-8">
                ...
                <div class="add_coloring_title">Добавьте изображение обложки</div>
                <form @submit="formSubmit" enctype="multipart/form-data">
                    <input type="file" class="form-control" v-on:change="imgPreview" name="avatar">
                    <button class="btn btn-light add_coloring_title" :disabled='isDisabled_button'>Отредактировать сказку</button>
                ...
                <button class="go_to_page" v-on:click="go_to_page()">Перейти на постраничное редактирование</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        get_start_fairy(inp)
        {
            axios
                .post('/get_edit_fairy',{
                    color_id:this.current_fairy_vue
                })
                .then(({ data }) => (
        ...
        formSubmit(e) {
            ...
            {
                this.alert=true;
                this.alert_arr.push('Заполните поле "Название сказки"');
                this.isActive_name=true;
            }
            if((this.coloring_name.length<5)&&(this.coloring_name!==""))
            {
                this.alert=true;
                this.alert_arr.push('Поле "Название сказки не должно быть короче 5 символов"');
                this.isActive_name=true;
            }
            if((this.coloring_name.length>25)&&(this.coloring_name!==""))
            {
                this.alert=true;
                this.alert_arr.push('Поле "Название сказки не должно быть длиннее 25 символов"');
                this.isActive_name=true;
            }
            if(this.description=="")
            {
            ...
            {
                this.alert=true;
                this.alert_arr.push('Поле "Описание сказки не должно быть короче 10 символов"');
                this.isActive_description=true;
            }
            if((this.description.length>300)&&(this.description!==""))
            {
                this.alert=true;
                this.alert_arr.push('Поле "Описание сказки не должно быть длиннее 300 символов"');
                this.isActive_description=true;
            }
            if(this.tag_list=='')
            {
            ...
            data.append('coloring_name', coloring_name);
             ...
             ,data,config)
                ...
                // .then(function (res) {
                //
                //     // window.location.href =('/admin/edit_coloring_success/fairy')
                // })
            .catch((error) => {
        ...
        go_to_page()
        {
            window.location.href =('/admin/add_fairy/'+this.current_fairy_vue+'/1')
        }
    }
}
</script>
