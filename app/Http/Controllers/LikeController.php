...
class LikeController extends Controller
{
    public function setLike(Request $request)
    {
        ...
        if(Auth::id())
        {
            ...
            $list_like = Like::where('post_id', $colored_id)->where('type_of_content', '1')->where('user_id', $userId)->get('type_of_like');
            if($list_like->isEmpty())
            {
                ...
                $list_like_count = LikeCounter::where('post_id', $colored_id)->where('type_of_content', '1')->get('count_of_like');
            ...
            else
            {
                if($list_like['0']['type_of_like']==$type_of_like)
                {
                    ...
                    $list_like = Like::where('post_id', $colored_id)->where('user_id', $userId)->where('type_of_content', '1')->delete();
                    if ($type_of_like=='1')
                    {
                    $list_like_count = LikeCounter::where('post_id', $colored_id)->where('type_of_content', '1')->get('count_of_like');
                    $list_like_count= $list_like_count['0']['count_of_like']-1;
                ...
                {
                    $list_like = Like::where('post_id', $colored_id)->where('user_id', $userId)->where('type_of_content', '1')->update(
                        [
